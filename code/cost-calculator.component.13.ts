...
export class CostCalculator {
  ...
  readonly addToCart = this.store.effect<number>(
    (quantity$) => quantity$.pipe(
      withLatestFrom([this.productType$]),
      tap( ([quantity, productType]) =>
        this.shoppingCartService.addToCart({ quantity, productType }))
    )
  )
